...
class SmartNameProcessor:
    ...
    def correct_text_vectorized(self, series):
        return series.apply(self.normalizer.normalize)  # همچنان apply اما hazm vectorized نیست، اما برای 3000 سریع
    ...
    def smart_score(self, f1, l1, f2, l2, org1="", org2="", bank1="", bank2="", post1="", post2="", mobile1="", mobile2=""):
        first_sim = fuzz.token_sort_ratio(f1, f2) / 100  # بهبود: token_sort برای ترتیب کلمات
        ...
        if self.settings.stop_first_names and (f1 in self.settings.stop_first_names or f2 in self.settings.stop_first_names):
            first_sim *= 0.8  # penalty ملایم‌تر و یک بار
        ...
        if mobile1 and mobile2:
            mobile1_clean = ''.join(filter(str.isdigit, str(mobile1)))[-11:]  # normalize به 11 digit آخر
    ...
    def find_similar_names(self, output_path="final_smart_similar_names.xlsx"):
        ...
        for idx, row in self.df.iterrows():
            ...
            if first_name or last_name:  # نگه داشتن حتی اگر یکی خالی باشد
                records.append((idx, first_name, last_name))
        
        results = []
        seen_pairs = set()
        
        # برای کوچک (<3000)، همه را مقایسه – سریع با rapidfuzz
        ...
        for i in range(len(records)):
            ...
            other_records = [(idx, f, l) for idx, f, l in records[i+1:]]  # فقط جلوتر برای جلوگیری duplicates
                ...
                if final_score >= self.settings.name_threshold:
                    ...
                    if pair_key not in seen_pairs:
                        ...
                        results.append([
                            ...
                            final_score * 100  # به درصد برای خوانایی
                        ])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Optimized Smart Name Processor for small datasets.")
    parser.add_argument("input_file", type=str, help="Path to input file.")
    parser.add_argument("--output_similar", type=str, default="final_smart_similar_names.xlsx", help="Output for similar names.")
    parser.add_argument("--name_threshold", type=float, default=0.80)
    parser.add_argument("--last_weight", type=float, default=0.45)
    parser.add_argument("--first_weight", type=float, default=0.2)
    parser.add_argument("--org_weight", type=float, default=0.25)
    parser.add_argument("--post_weight", type=float, default=0.05)
    parser.add_argument("--mobile_weight", type=float, default=0.1)
